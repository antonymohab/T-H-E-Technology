...
const App: React.FC = () => {
  ...
  const handleDownloadItemPDF = async () => {
    ...
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    doc.text("ITEM INVENTORY REPORT", 50, 20);
    ...
    if (catalogDateFrom && catalogDateTo) {
      doc.text(`Date Range: ${catalogDateFrom} to ${catalogDateTo}`, 50, 34);
    }

    let currentY = 50;

    // Draw Item Image & Basic Info
    if (itemImg) {
      // Draw image on the left
      doc.addImage(itemImg, 'JPEG', 14, currentY, 60, 60);
      
      // Draw text next to image
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(0, 0, 0);
      doc.text(brandName.toUpperCase(), 80, currentY + 10);
      
      doc.setFontSize(24);
      doc.text(catalogSelectedModel.model_name.toUpperCase(), 80, currentY + 22);

      // Draw Counts
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(0, 0, 0);
      doc.text(`Total Units: ${totalCount}`, 80, currentY + 35);
      
      doc.setTextColor(16, 185, 129); // Emerald 500
      doc.text(`Available: ${availableItems.length}`, 80, currentY + 43);
      
      doc.setTextColor(245, 158, 11); // Amber 500
      doc.text(`Reserved: ${reservedItems.length}`, 80, currentY + 51);
      
      doc.setTextColor(239, 68, 68); // Red 500
      doc.text(`Maintenance: ${maintenanceItems.length}`, 80, currentY + 59);

      doc.setTextColor(220, 38, 38); // Darker Red
      doc.text(`Broken: ${brokenItems.length}`, 80, currentY + 67);
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(0, 0, 0);
      doc.text(brandName.toUpperCase(), 14, currentY);
      
      doc.setFontSize(24);
      doc.text(catalogSelectedModel.model_name.toUpperCase(), 14, currentY + 12);

      // Draw Counts
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(0, 0, 0);
      doc.text(`Total Units: ${totalCount}`, 14, currentY + 25);
      
      doc.setTextColor(16, 185, 129); // Emerald 500
      doc.text(`Available: ${availableItems.length}`, 14, currentY + 33);
      
      doc.setTextColor(245, 158, 11); // Amber 500
      doc.text(`Reserved: ${reservedItems.length}`, 14, currentY + 41);
      
      doc.setTextColor(239, 68, 68); // Red 500
      doc.text(`Maintenance: ${maintenanceItems.length}`, 14, currentY + 49);

      doc.setTextColor(220, 38, 38); // Darker Red
      doc.text(`Broken: ${brokenItems.length}`, 14, currentY + 57);

      currentY += 75;
    }

    // Draw Table
    const tableData = modelItems.map(item => [
      item.serial_number || 'N/A',
      getSerialNumberStatus(item).toUpperCase(),
      item.last_maintenance_date || 'Never'
    ]);
    ...
    autoTable(doc, {
      ...
      didParseCell: function(data) {
        if (data.section === 'body' && data.column.index === 1) {
          ...
          } else if (status === 'IN USE' || status === 'RESERVED') {
            data.cell.styles.textColor = [245, 158, 11];
            data.cell.styles.fontStyle = 'bold';
          } else if (status === 'BROKEN' || status === 'MAINTENANCE') {
                        ...
                        <div key={category} className="space-y-8">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8">
                            {Object.values(items.reduce((acc: any, item: any) => {
                              ...
                              <div key={idx} className="bg-white p-8 rounded-[2.5rem] border border-zinc-100 flex items-center gap-8 shadow-sm hover:shadow-xl transition-all group/item">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {group.serials.map((sn: string, i: number) => (
                                  ...
                                  {isExpanded && (
                                    <div className="p-2 space-y-1 bg-white">
                                      {brandModels.map(model => (
                                        <div key={model.id} className="flex items-center justify-between p-3 hover:bg-zinc-50 rounded-xl transition-colors group">
                                          <div className="flex items-center gap-3">
                                            {model.image_url ? (
                                    ...
                                    <div key={category} className="space-y-4">
                                       ...
                                       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                          {Object.values(itemsByCategory[category].reduce((acc: any, bi: any) => {
                                            ...
                                            <div key={idx} className="flex items-start gap-4 p-4 bg-zinc-50 rounded-2xl border border-zinc-100 group hover:border-[#8cbcf3] transition-all">
